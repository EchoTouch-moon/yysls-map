...
├── probe_stratify.py          ← H-NEX-003AG walkable-minority stratification probe
...
- H-NEX-003AG Stratify Walkable Minority：DONE（Gate = **STRATIFICATION_PARTIAL**；~3.5% 高覆盖少数块在三归档复现（112/100/112 块），在小 proto 中富集 ~5 倍（instrs 中位 23-25 vs 69-71，block_size ~850 vs ~2250，tail_len ~640 vs ~1700），但最优单规则精度仅 0.18-0.22（< 0.70 门槛）→ 不可分离；post-code 字节文法线按预注册终结收口，交付物 = 0x04/0x14 标记普查与三态开局普查）
